...
export class PikachuVolleyball {
  ...
  constructor(stage, resources) {
    ...
    this.keyboardArray = [];
    this.setKeyboardBindings(this.getSavedKeyboardBindings());
  ...
  getSavedKeyboardBindings() {
    const defaults = this.getDefaultKeyboardBindings();
    try {
      const saved = JSON.parse(localStorage.getItem('pikachu-volleyball-controls-v1'));
      if (!saved?.player1 || !saved?.player2) return defaults;
      return {
  ...
  setKeyboardBindings(bindings) {
    this.keyboardArray.forEach((keyboard) => keyboard.unsubscribe());
    this.keyboardArray = [
      new PikaKeyboard(
        bindings.player1.left,
        bindings.player1.right,
        bindings.player1.up,
        bindings.player1.down,
        bindings.player1.powerHit
      ),
      new PikaKeyboard(
        bindings.player2.left,
        bindings.player2.right,
        bindings.player2.up,
        bindings.player2.down,
        bindings.player2.powerHit
      ),
    ];
  }
